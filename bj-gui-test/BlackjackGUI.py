...
class GameWindow(QMainWindow):
    def __init__(self):
        ...
        self.bot_stayed = False
        self.bot2_stayed = False
    ...
    def bot_turn(self, current_player):
        if current_player == 'bot':
            ...
            # bot2 strategy
            while self.get_score(self.bot2) < 21:
                if self.get_score(self.bot2) < 17:
                    self.bot_hit('bot2')
                else:
                    # try_index = random.randint(1,10)
                    # if try_index <= 3 and self.get_score(self.bot) < 21:
                    ...
                    self.bot2_stayed = True
                    self.bot_turn('bot')
                    break
                QtTest.QTest.qWait(1000)

            # if self.get_score(self.bot2) == 21:
            #     self.end()
        
    def bot_hit(self, current_player):
        if current_player == 'bot' and self.bot_stayed == False and self.bot2_stayed == True:
            #Draw 1 card for bot
            self.bot.append(self.game.cards.pop())
            self.display_bot_face(self.bot)
            if self.get_score(self.bot) > 21:
                self.bust = True
                self.end()
        elif current_player == 'bot2' and self.bot2_stayed == False and self.bot_stayed == False:
            # bot2 strategy
            #Draw 1 card for bot
            self.bot2.append(self.game.cards.pop())
            self.display_bot2_face(self.bot2)
    ...
    def bot_stay(self, current_player):
        if current_player == 'bot':
            #Pass and check the score
            ...
            self.bot_stayed = True
            self.end()
        else:
            # bot2 strategy
            #Pass and check the score
            score = self.get_score(self.bot2)
            if score > 21:
                self.bust = True
            #reveal the bot's cards
            self.end()
    ...
    def display_bot2_face(self,player):
        #display the card at the position (x,y)
        for i in range(len(player)):
            card = QLabel(self)
            card.setPixmap(QPixmap(card_images_path+'/'+str(player[i].value)+'_of_'+player[i].suit+'.png').scaled(200, 245, Qt.KeepAspectRatio))
            #resize the image to 100*150 and keep the card at the bottom of the window
            card.setGeometry(1000 + i*50, 450, 200, 245)
            self.layout().addWidget(card)
    ...
    def clear_bot2_face(self):
        #clear the card at the position (x,y)
        for i in range(len(self.bot2)):
            card = QLabel(self)
            card.setPixmap(QPixmap(card_images_path+'/back2.png').scaled(200, 245, Qt.KeepAspectRatio))
            #resize the image to 100*150 and keep the card at the bottom of the window
            card.setGeometry(1000 + i*50, 450, 200, 245)
